...
namespace Crucero.Infraestructure.Repository.Implementations
{
    public class RepositoryDestino : IRepositoryDestino
    {
        ...
        public RepositoryDestino(WaddelsContext context)
        {
            _context = context;
        }

        public Task<Destino> FindByIdAsync(int id)
        {
            throw new NotImplementedException();
        }
        public async Task<ICollection<Destino>> ListAsync()
        {
            //Select * from Autor
            var collection = await _context.Set<Destino>().ToListAsync();
            return collection;
        }
    }
}
